...
/*void q6 () {
	...
	Attribute s_nationkey = {"s_nationkey", Int};
	Schema groupby_schema ("catalog", "supplier");
	printf("%d join schema\n", groupby_schema.GetNumAtts()); 

	GroupBy G;
		// _s (input pipe)
		Pipe _out (100);
		Function func;
			char *str_sum = "(s_nationkey)";
			get_cnf (str_sum, &groupby_schema, func);
			func.Print ();
			printf("%d join schema\n", groupby_schema.GetNumAtts()); 
			int atts[1] = {3}; 
			Type types[1] = {Int}; 
			OrderMaker grp_order;
			grp_order.setAttributes(atts, types, 1);
	G.Use_n_Pages (3);
	printf("In test.cc: in: %ld, out: %ld \n", &_s, &_out);
	G.Run (_s, _out, grp_order, func);
	// SF_s.WaitUntilDone (); 
	G.WaitUntilDone ();
...
void q7(){
	...
	char *pred_ps = "(s_nationkey = s_nationkey)";
	CNF cnf;
	Record literal;
	get_cnf(pred_ps,&join_sch,cnf,literal);
	// Schema schema1("catalog","partsupp");
	// char *pred_ps = "(ps_suppkey = ps_suppkey)";
	// CNF cnf1;
	// get_cnf(pred_ps,&schema1,cnf1,literal);
	OrderMaker grp_order;
	int *AttstoKeep=new int[1];
	AttstoKeep[0]=3;
	Type *attType=new Type[1];
	...
	grp_order.setAttributes(AttstoKeep,attType,1);
	DBFile dbfile;
	// dbfile.Create("test5.bin",heap,NULL);
	// dbfile.Load(join_sch,"test4.txt");
	// dbfile.Close();
	// dbfile.Open("partsupp.bin");
	// SelectFile sf;
	// Pipe out(100);
	// // sf.Run(dbfile,out,cnf,literal);
	// // sf.WaitUntilDone();
	// sf.Run(dbfile,out,cnf1,literal);
	// WriteOut wo;
	// FILE *file=fopen("test3.txt","w+");
	// wo.Run(out,file,schema1);
	// sf.WaitUntilDone();
	// wo.WaitUntilDone();
	// dbfile.Open("test5.bin");
	// SelectFile sf;
	// Pipe out(100);
	// sf.Run(dbfile,out,cnf,literal);
	// // sf.WaitUntilDone();
...
void q8(){
	cout << " query8 \n";
	char *pred_s = "(s_nationkey = s_nationkey)";
	// init_SF_s (pred_s, 100);
	// SF_s.Run (dbf_s, _s, cnf_s, lit_s); // 10k recs qualified
	...
	CNF cnf;
	Record literal;
	get_cnf(pred_s,&join_sch,cnf,literal);
	SelectFile sf;
	DBFile dbfile;
	dbfile.Open("test5.bin");
	Pipe out(100);
	...
	Schema sum_sch ("sum_sch", 1, &DA);
	// int cnt = clear_pipe (_out, &sum_sch, true);
	// cout << " query8 returned sum for " << cnt << " groups (expected 25 groups)\n"; 
}
